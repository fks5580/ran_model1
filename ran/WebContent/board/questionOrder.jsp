...
<%
	...
	List<boardBean> list = boardDAO.getBoardList(startRow, pageSize);
%>
...
%>
				<tr onclick="location.href='contentOrder.jsp?no=<%=bean.getNo()%>&pageNum=<%=pageNum%>'">
					...
					<% 
						int wid = 0;
						if(bean.getMain_tab()>0){
							wid = bean.getMain_tab() * 5;
					%>	
...
%>
				...
				<form action="selquestionOrder.jsp" method="post">
					...
					<select name=sel_qorder id="sq">
						...
						<input type="text" name="qorder" id="sq"> 
				
				<button type="submit" class="btn hvr-hover" id="sq">검색</button>
...
%>
			<a href="questionOrder.jsp?pageNum<%=startPage-pageBlock%>">[이전]</a>	
<% 
					}
					for(int i =startPage; i<=endPage;i++){
%>
			<a href="questionOrder.jsp?pageNum=<%=i %>">[<%=i %>]</a>
<%	
						}
					if(endPage < pageCount){
%>
			<a href="questionOrder.jsp?pageNum=<%=startPage+pageBlock %>">[다음]</a>
<%
					}
				}
%>
