...
<%
	...
	replyboardBean rBean = replyboardDAO.getReply(no,reply_no);
...
<!-- 왼쪽메뉴 -->
	...
	<!-- Start 게시글 입력 -->
		...
		<form action="updatereplyOrderPro.jsp" method="post">
			<input type="hidden" id="reply_date" name="date" value="<%=rBean.getReply_date()%>">
			<input type="hidden" id="no" name="no" value="<%=rBean.getNo() %>">
			<input type="hidden" id="pageNum" name="pageNum" value="<%=pageNum %>">
			<input type="hidden" id="reply_no" name="reply_no" value="<%=rBean.getReply_no()%>">
			<table id="">
				<tr>
					<td>글쓴이</td>
					<td><input type="text" name="reply_writer" id="reply_writer" value="<%=rBean.getReply_writer()%>" readonly></td>
					<td>비밀번호</td>
					<td><input type="password" name="reply_pw" id="reply_pw"></td>
				</tr>
				<tr>
					<td>댓글내용</td>
					<td><textarea  name="reply_content" id="reply_content" rows="13" cols="40"><%=rBean.getReply_content()%></textarea></td>
				</tr>
			</table>
				<button type="submit" class="btn">댓글수정</button>			
			</form>
