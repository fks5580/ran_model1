...
<!-- 왼쪽메뉴 -->
	...
	<!-- Start 게시글 입력 -->
		...
		<form action="updatewriteOrderPro.jsp" method="post">
			<input type="hidden" id="date" name="date" value="<%=b.getDate()%>">
			<input type="hidden" id="main_seq" name="main_seq" value="<%=b.getMain_seq()%>">
			<input type="hidden" id="main_tab" name="main_tab" value="<%=b.getMain_tab()%>">
			<input type="hidden" id="sub_seq" name="sub_seq" value="<%=b.getSub_seq()%>">
			<input type="hidden" id="readcount" name="readcount" value="<%=b.getReadcount()%>">
			<input type="hidden" id="no" name="no" value="<%=b.getNo() %>">
			<table id="">
				<tr>
					<td>글쓴이</td>
				...
				<tr>
					<td>글내용</td>
					<td><textarea  name="content" id="content" rows="13" cols="40"><%=b.getContent()%></textarea></td>
				</tr>
			</table>
				...
				<%
						if(id.equals(writer)){	
				%>
						<input type="button"  value="글삭제" class="btn" onclick="location.href='deleteOrder.jsp?no=<%=request.getParameter("no")%>'">
				<%	
					}
				%>
				
				<input type="button" class="btn"  value="글목록" onclick="location.href='questionOrder.jsp'">			
			</form>
